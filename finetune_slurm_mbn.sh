#!/bin/bash
#SBATCH --job-name=Mbn_Mo_FineTune           # Job name
#SBATCH --partition=small_job      # Partition name
#SBATCH --nodes=1                         # Number of nodes
#SBATCH --gres=gpu:1                      # Number of GPUs
#SBATCH --cpus-per-task=8                 # Number of CPU cores per task
#SBATCH --time=1-00:00:00              # Time limit
#SBATCH --output=logs/finetune_slurm_mbn_ocean_puck_lagoon.out  # Standard output log
#SBATCH --error=logs/finetune_slurm_mbn_ocean_puck_lagoon.err   # Error log
...
MODEL=mocogeo                                 # Model name
TRAIN_BATCH_SIZE=1                    # Training batch size
VAL_BATCH_SIZE=1                     # Validation batch size
LEARNING_RATE=1e-4 #1e-5                  # Learning rate
EPOCHS=10                              # Number of epochs

PRETRAINED_MODEL="./results/trains/moco-geo-ocean/version_6/checkpoints/epoch=199-step=19800.ckpt"
